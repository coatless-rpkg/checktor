...
is bookkeeping shared with every other check. The version that ships adds one
...
A finished `diagnose_*` function does nothing until `checktor()` knows to call
it. `register_check()` connects the two at run time, with no edit to checktor's
own source:
...
- The function returns a `checktor_check_result()`, the same shape as every
  shipped check, so `run_checks()` handles its error catching and `$passed`
  bookkeeping.
...
> way the shipped checks are: drop `diagnose_my_check()` into the right
> `R/diagnostics-*.R` file, add one line to that file's
> `diagnose_<category>_issues()` function inside its `run_checks(list(...))` call
> (`my_check = function(p, v) diagnose_my_check(p, v, parsed = parsed)`), and give
...
`allow` mutes a specific finding (a whole check, or `check:substring`), which is
the escape hatch a green `checkup()` gate needs; and `disable` skips a check
outright. Reach for a hand-written check when the rule is genuinely yours; reach
for `Config/checktor/*` when you only need to teach or quiet a check that already
ships.
