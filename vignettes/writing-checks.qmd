---
title: "Writing Your Own checktor Checks"
vignette: >
  %\VignetteIndexEntry{Writing Your Own checktor Checks}
...
CRAN-style rules that aren't worth upstreaming. This vignette walks through
the helpers in `R/ast.R` and shows how to author a new check against the
parsed AST in a few lines of XPath.
...
every check via this hidden argument, so 11 checks against a 200-file
package mean 200 parses, not 2200.
...
`list(file, xml, error)`. A parse failure becomes an `error` slot instead of
crashing the run.
...
and ships in checktor as `diagnose_sys_setenv_no_reset`. Here's the
implementation:
...
Twenty lines. The interesting part is the XPath predicate — everything else
...
A common bug is treating `parent::expr` as the call expr — it's actually the
...
To plug a new check into `checktor()`, add a `diagnose_<name>` function to
the appropriate `R/diagnostics-*.R` file and add an entry to that file's
