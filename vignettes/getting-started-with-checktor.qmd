---
title: "Getting Started with checktor - Your Package Doctor"
vignette: >
  %\VignetteIndexEntry{Getting Started with checktor - Your Package Doctor}
...
`checktor` is the package doctor for R packages heading to CRAN. Like a medical professional, it examines your package for common ailments and provides specific treatment recommendations to ensure a healthy CRAN submission.

The package provides automated diagnostics for CRAN submission issues that are not caught by standard `R CMD check`, helping you identify and fix problems before submission.
...
The main function `checktor()` runs a comprehensive diagnostic on your package:
...
2. **Seed Setting**: Hardcoded `set.seed()` calls without user control
3. **Print/Cat Usage**: Unsuppressable console output
4. **Option Changes**: Changing `options()`, `par()`, or `setwd()` without reset
...
8. **installed.packages()**: Using slow `installed.packages()` function
...
1. **Software Names**: Package and software names should be in single quotes
...
3. **Example Structure**: Appropriate use of `\dontrun`, `\donttest`, etc.
...
## Inspecting Results as Data

The printed report is for reading. When you want to *work* with the findings,
the accessor functions return plain data frames you can filter, sort, or drop
into a report of your own -- no navigating nested lists.

```{r eval=FALSE}
results <- checktor()

summary(results)   # one row per category: checks, passed, failed, issues
tidy(results)      # one row per check that ran
issues(results)    # one row per individual issue, with file and line
```

Three predicates answer the common questions directly:

```{r eval=FALSE}
is_healthy(results)      # TRUE if nothing was flagged
n_issues(results)        # total number of issues found
failed_checks(results)   # names of the checks that failed
```

`as.data.frame(results)` is equivalent to `tidy(results)`, and every accessor
also works on a single category (`issues(results$code_issues)`) or a single
check.

## Quick Health Checks

For CI/CD pipelines or quick verification, use the `checkup()` function:
...
The `prescribe()` function provides code examples showing exactly how to fix common issues.
...
`checkup()` returns a single `TRUE`/`FALSE`, so it drops straight into a build
step as a quality gate. For a ready-to-use GitHub Actions workflow -- including
how to install `checktor` with `setup-r-dependencies` and surface the diagnosis
in the log when a build fails -- see the
...
All output avoids medical emojis and uses professional, accessible language suitable for both beginners and experienced developers.
...
Found a new CRAN diagnostic that should be included? The package is designed to be easily extensible. Please open an issue or submit a pull request!
...
The package doctor is here to help keep your package healthy and ready for CRAN!
