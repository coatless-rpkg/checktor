...
For now, `checktor` lives on GitHub. Once it reaches CRAN you will be able to
`install.packages("checktor")`, but until then you install the development
version.
...
> **On your own package**, the call is simply `checktor()`. Run from the
> package root, it examines everything in place.
...
![The one `checktor_results` object fans out into three plain data frames. `summary()` gives 5 rows, one per category. `issues()` gives 10 rows, one per issue, carrying the file and line. `tidy()` gives 45 rows, one per check, whether it passed or not.](figures/result-shapes-light.svg){.ct-fig .ct-light}
