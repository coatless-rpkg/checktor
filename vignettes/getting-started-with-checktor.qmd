...
![The one `checktor_results` object fans out into three plain data frames. `summary()` gives 5 rows, one per category. `issues()` gives 8 rows, one per issue, carrying the file and line. `tidy()` gives 51 rows, one per check, whether it passed or not.](figures/result-shapes-light.svg){.ct-fig .ct-light}
...
its alias. Its `skipped` column marks any check that did not run, such as the URL
fetch when you are not at the console, so a check that sat out never reads as one
that passed. Three predicates answer the yes/no questions directly:
