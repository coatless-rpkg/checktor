...
* Three checks were dropped from the default run because no authority supports
  them, and each stays exported and callable for anyone who wants it.

  - `description_bare_r` demanded that every bare `R` in the `Description` be
    single-quoted. Writing R Extensions reserves single quotes for other packages
    and external software, and `R` is the host language, so most packages that
    mention it write it bare and stay on CRAN.
